...
client.on('ready', () => {
  console.log('켰다.');
  client.user.setPresence({ game: { name: '!코로나 의심시 국번없이 1339' }, status: 'online' })

  let state_list = [
    '!시리야',
    '구매문의 Apple#7777',
    '배너문의 Apple#7777',
    '1호점 https://discord.gg/6V5vQKr',
    '2호점 https://discord.gg/fNkYZHg',
  ]
...
client.on('message', (message) => {
  ...
  if(message.content == '!계좌') {
    return message.reply('759902-00-056835 국민 ㅂㅈㅎ');
  }

  if(message.content == '!서버정보') {
    let embed = new Discord.RichEmbed()
    let img = 'https://ibb.co/sPgPsLK';
    var duration = moment.duration(client.uptime).format(" D [일], H [시간], m [분], s [초]");
    embed.setColor('#186de6')
    embed.setAuthor('시리', img)
  ...
  if(message.content == '!애플') {
    let img = 'https://i.postimg.cc/K8dh6GLr/20200723-160158.png';
    let embed = new Discord.RichEmbed()
      .setTitle('애플은? ')
      .setURL('https://ibb.co/sPgPsLK')
      .setAuthor('애플', img, 'https://ibb.co/sPgPsLK')
      .setThumbnail(img)
      .addBlankField()
      .addField('[나이] : 비밀')
      .addField('[키] : 비밀', )
    ...
    let helpImg = 'https://i.postimg.cc/K8dh6GLr/20200723-160158.png';
    let commandList = [
      {name: '!시리야', desc: '도움이다'},
      {name: '!계좌', desc: '특정 지역에서만 가능'},
      {name: '!서버정보', desc: '서버 정보'},
      {name: '!청소', desc: '텍스트 지움'},
      {name: '!초대코드', desc: '해당 채널의 초대 코드 표기'},
      {name: '!초대코드2', desc: '봇이 들어가있는 모든 채널의 초대 코드 표기'},
      {name: '!무료봇', desc: 'https://discord.gg/EhyWJZP'},
    ];
    let commandStr = '';
    let embed = new Discord.RichEmbed()
      .setAuthor('시리', helpImg)
      .setColor('#186de6')
      .setFooter(`시리 ❤️`)
      .setTimestamp()
